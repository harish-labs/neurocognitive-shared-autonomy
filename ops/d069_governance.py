...
EXPECTED_BASE = "aecf11a2886624b9faff53f5b9a70d5cb494ae1b"
...
def commit_push(path: str, message: str) -> str:
...
if head != EXPECTED_BASE:
    raise SystemExit(f"canonical base moved: expected {EXPECTED_BASE}, got {head}")

# 1. Record D-069 without rewriting prior decisions.
p = Path("DECISIONS.md")
text = p.read_text(encoding="utf-8-sig")
if "## D-069 — Interruptible Navigation Execution Contract" in text:
    raise SystemExit("D-069 already exists")
marker = "# 3. UNRESOLVED DECISIONS"
if marker not in text:
    raise SystemExit("unresolved-decisions marker not found")

decision = r'''## D-069 — Interruptible Navigation Execution Contract
...
'''
p.write_text(text.replace(marker, decision + marker, 1), encoding="utf-8")
commit_push("DECISIONS.md", "Approve interruptible navigation contract D-069")
...
'''
Path("CURRENT_TASK.md").write_text(current_task, encoding="utf-8")
commit_push("CURRENT_TASK.md", "Authorize M5-T03 stepwise navigation runtime")
...
commit_push("PROJECT_STATE.md", "Advance project state to M5-T03")
