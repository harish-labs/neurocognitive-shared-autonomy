...
D083_PARTICIPATION_MANIFEST_VERSION = "m7-d083-sequential-participation-v1"
...
@dataclass(frozen=True)
class SequentialParticipationRecord:
    subject_id: int
    t1_episode_count: int
    t2_episode_count: int
    balanced_sequential_eligible: bool
    exclusion_reason: str | None


@dataclass(frozen=True)
class SequentialParticipationManifest:
    manifest_version: str
    policy_ids: tuple[str, ...]
    split_manifest_sha256: str
    partition_name: str
    frozen_partition_subject_ids: tuple[int, ...]
    records: tuple[SequentialParticipationRecord, ...]
    included_subject_ids: tuple[int, ...]
...
def construct_episode_manifest(source_metadata: pd.DataFrame) -> EpisodeManifest:
    ...
    manifest = EpisodeManifest(
        ...
        policy_ids=("D-051", "D-052", "D-053", "D-054", "D-074", "D-077", "D-078", "D-080", "D-081", "D-082", "D-083"),
...
def build_sequential_participation_manifest(
    episode_manifest: EpisodeManifest,
    *,
    split_manifest_sha256: str,
    partition_name: str,
    frozen_partition_subject_ids: Sequence[int],
    experiment_families: Sequence[str] = ("E3", "E4", "E6", "E7", "E9"),
) -> SequentialParticipationManifest:
    """Apply D-083 only after split freeze; never mutate frozen membership."""

    validate_episode_manifest(episode_manifest)
    if len(split_manifest_sha256) != 64:
        raise EpisodeConstructionError("D-083 participation requires a frozen split hash.")
    frozen = tuple(int(value) for value in frozen_partition_subject_ids)
    if not frozen or len(set(frozen)) != len(frozen):
        raise EpisodeConstructionError("Frozen partition subject IDs must be non-empty and unique.")
    counts: dict[int, dict[str, int]] = {subject_id: {"left": 0, "right": 0} for subject_id in frozen}
    for episode in episode_manifest.episodes:
        if episode.subject_id in counts:
            counts[episode.subject_id][episode.intended_class] += 1
    records = []
    for subject_id in frozen:
        left = counts[subject_id]["left"]
        right = counts[subject_id]["right"]
        eligible = left >= 1 and right >= 1
        if eligible:
            reason = None
        elif left == 0 and right == 0:
            reason = "no_valid_d081_t1_or_t2_episode"
        else:
            reason = "valid_d081_episodes_for_only_one_intended_class"
        records.append(SequentialParticipationRecord(subject_id, left, right, eligible, reason))
    included = tuple(item.subject_id for item in records if item.balanced_sequential_eligible)
    excluded = tuple(item.subject_id for item in records if not item.balanced_sequential_eligible)
    manifest = SequentialParticipationManifest(
        manifest_version=D083_PARTICIPATION_MANIFEST_VERSION,
        policy_ids=("D-079", "D-081", "D-082", "D-083"),
        split_manifest_sha256=split_manifest_sha256,
        partition_name=str(partition_name),
        frozen_partition_subject_ids=frozen,
        records=tuple(records),
        included_subject_ids=included,
        excluded_subject_ids=excluded,
        experiment_families=tuple(str(value) for value in experiment_families),
        no_replacement=True,
    )
    validate_sequential_participation_manifest(manifest)
    return manifest


def validate_sequential_participation_manifest(manifest: SequentialParticipationManifest) -> None:
    if manifest.manifest_version != D083_PARTICIPATION_MANIFEST_VERSION or not manifest.no_replacement:
        raise EpisodeConstructionError("D-083 participation manifest version/no-replacement contract changed.")
    frozen = set(manifest.frozen_partition_subject_ids)
    included = set(manifest.included_subject_ids)
    excluded = set(manifest.excluded_subject_ids)
    if included & excluded or included | excluded != frozen:
        raise EpisodeConstructionError("D-083 participation must preserve every frozen partition subject.")
    if tuple(item.subject_id for item in manifest.records) != manifest.frozen_partition_subject_ids:
        raise EpisodeConstructionError("D-083 records must preserve frozen subject order exactly.")
    for item in manifest.records:
        expected = item.t1_episode_count >= 1 and item.t2_episode_count >= 1
        if item.balanced_sequential_eligible != expected:
            raise EpisodeConstructionError("D-083 participation requires at least one episode for each class.")
        if expected != (item.exclusion_reason is None):
            raise EpisodeConstructionError("D-083 sequential exclusion reason is inconsistent.")


def freeze_sequential_participation_manifest(
    manifest: SequentialParticipationManifest,
    path: str | Path,
) -> str:
    validate_sequential_participation_manifest(manifest)
    ...
    text = json.dumps(_plain(asdict(manifest)), sort_keys=True, separators=(",", ":"), allow_nan=False) + "\n"
    if destination.exists() and destination.read_text(encoding="utf-8") != text:
        raise EpisodeConstructionError("Frozen D-083 participation manifest already has different content.")
    if not destination.exists():
        destination.write_text(text, encoding="utf-8", newline="\n")
    return _file_sha256(destination)


def sequential_participation_manifest_from_mapping(
    payload: Mapping[str, Any],
) -> SequentialParticipationManifest:
    try:
        manifest = SequentialParticipationManifest(
            manifest_version=str(payload["manifest_version"]),
            policy_ids=tuple(str(value) for value in payload["policy_ids"]),
            split_manifest_sha256=str(payload["split_manifest_sha256"]),
            partition_name=str(payload["partition_name"]),
            frozen_partition_subject_ids=tuple(int(value) for value in payload["frozen_partition_subject_ids"]),
            records=tuple(
                SequentialParticipationRecord(
                    subject_id=int(item["subject_id"]),
                    t1_episode_count=int(item["t1_episode_count"]),
                    t2_episode_count=int(item["t2_episode_count"]),
                    balanced_sequential_eligible=bool(item["balanced_sequential_eligible"]),
                    exclusion_reason=(None if item["exclusion_reason"] is None else str(item["exclusion_reason"])),
                )
                for item in payload["records"]
            ),
            included_subject_ids=tuple(int(value) for value in payload["included_subject_ids"]),
            ...
            no_replacement=bool(payload["no_replacement"]),
        )
    except (KeyError, TypeError, ValueError) as exc:
        raise EpisodeConstructionError("Invalid D-083 participation manifest mapping.") from exc
    validate_sequential_participation_manifest(manifest)
    return manifest
